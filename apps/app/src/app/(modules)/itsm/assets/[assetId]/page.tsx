...
async function getTenantId() {
  ...
  if (!membership?.tenant_id) redirect("/apps");
...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex rounded-full border hi5-border bg-black/5 dark:bg-white/5 px-2.5 py-1 text-xs font-semibold">
      {children}
    </span>
  );
}
        ...
        <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4">
          <div>
            <div className="flex flex-wrap gap-2">
              ...
              <Badge>Source: {asset.source}</Badge>
              {linkedDevice ? <Badge>{linkedDevice.online ? "Control online" : "Control offline"}</Badge> : null}
            </div>
        ...
        <div className="hi5-card p-4 space-y-3">
          ...
          {linkedDevice ? (
            ...
            <div className="rounded-2xl border hi5-border bg-black/5 dark:bg-white/5 p-4 text-sm opacity-75">
              This asset is not linked to a live Control device yet. Later, assets can be matched automatically by serial number,
              hostname, Intune device ID or agent identity.
            </div>
          )}
        </div>
